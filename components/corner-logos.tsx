...
export function CornerLogos() {
  ...
  // Make the 25-years logo a bit smaller on the home page
  const rightLogoWidth = showLuminusLogoOnRight ? 144 : 64
  const rightLogoHeight = showLuminusLogoOnRight ? 144 : 64
  const rightLogoClassName = showLuminusLogoOnRight
    ? "h-16 w-auto max-h-16 object-contain sm:h-36 sm:max-h-36"
    : "h-7 w-auto max-h-7 object-contain sm:h-16 sm:max-h-16"
  ...
  const rightLogoWrapperClassName = showLuminusLogoOnRight
    ...
    : "fixed top-5 right-3 z-50 flex h-10 sm:h-20 items-center justify-center"
